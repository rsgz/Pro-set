...
<template>
  <el-container style="height: 100vh; background-color: #303841; color: #fff;">
    <!-- 头部 -->
    <el-header style="background-color: #3a4750; padding: 20px;">
      <h1 style="margin: 0;padding: 0;">书签集合</h1>
    </el-header>
    ...
    <el-main>
      ...
      <el-row :gutter="20" style="margin-bottom: 20px;">
        ...
        <el-col :span="6">
          <el-button type="primary" @click="fetchBookMarks">搜索</el-button>
          <el-button type="success" @click="openDialog">添加书签</el-button>
        </el-col>
      </el-row>
...
const openDialog = (bookMark = null) => {
  console.log("bookMark.value",bookMark.value);
  console.log("!bookMark.value",!bookMark.value);
  console.log("!!bookMark.value",!!bookMark.value);
  ...
  isEdit.value = !!bookMark.value;  // 如果 bookMark 不是 null 或 undefined，则 isEdit.value 被设置为 true，表示当前操作是在编辑模式下进行
  ...
  currentBookMark.value = bookMark.value ? { ...bookMark } : { name: '', url: '', source: '', purpose: '', tag: '' };
...
const saveBookMark = async () => {
  ...
  if (isEdit.value) {
    ...
    const response = await axios.post('/api/bookmarks', currentBookMark.value,{
      headers: {
