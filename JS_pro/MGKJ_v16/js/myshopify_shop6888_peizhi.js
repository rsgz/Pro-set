...
document.getElementById('delete-button').addEventListener('click', () => deleteSpecificData(['column1', 'column2', 'column3', 'column4', 'column5', 'column6', 'column7', 'column8', 'column9', 'column10', 'column11']))
...
//         // 等待1秒
//         await delay(500);
//         console.log('数量', $('td, td input').length);
...
// 监听所有input 然后 便捷粘贴 一列 数据
document.addEventListener('DOMContentLoaded', async function () {
    console.log("正在监听");

    await delay(ms = 2000)  // 就是因为这里加上了两秒 拯救了 这个功能 否则 只是前面两排的td有用
    // 假设只有一个表格需要这个功能，且表格的id是'myTable'
    var table = document.getElementById('myTable');

    // 监听每个input的粘贴事件
    table.querySelectorAll('input').forEach(function (input) {
        input.addEventListener('paste', function (e) {
            console.log("input 个数:", table.querySelectorAll('input').length);

            e.preventDefault(); // 阻止默认粘贴行为
            var data = e.clipboardData || window.clipboardData;
            var pastedData = data.getData('Text');
            ...
            rows.forEach(function (rowData, index) {
                console.log(rowData, index); // https://bbmb.fun  0
                table.rows[index + rowIndex].cells[cellIndex].querySelector('input').value = rowData || ''; // 设置input的值为对应的单元格数据;
                // var cells = rowData.split('\t'); // 假设单元格数据以制表符分隔
                // var cells = rowData.split('\t'); // 假设单元格数据以制表符分隔
                // if (table.rows[index + rowIndex] && table.rows[index + rowIndex].cells[cellIndex]) {
                //     table.rows[index + rowIndex].cells[cellIndex].querySelector('input').value=cells[cellIndex] || ''; // 设置input的值为对应的单元格数据;
                // }
            });
        });
    });
...
function saveData() {
    ...
    var input11_arr = Array.from(document.querySelectorAll('.column11'), (input) => input.value);
...
function showData() {
    ...
    chrome.storage.local.get(['column1', 'column2', 'column3', 'column4', 'column5', 'column6', 'column7', 'column8', 'column9', 'column10', 'column11'], function (data) {
        // 检查是否成功检索到数据
        if (chrome.runtime.lastError) {
        ...
        const input11_arr = data.column11;
        ...
        Array.from(document.querySelectorAll('.column11')).forEach((input, index) => { input.value = input11_arr[index]; input.classList.add('text-center'); });
...
function add_hang_n(numRows, td) {
    ...
    */
    ...
    for (let i = 0; i < numRows; i++) {
        ...
        input11.setAttribute("class", "column11");
