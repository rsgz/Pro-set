...
$(function () {
    if (window.location.href.startsWith(doc360_url)) {
        ...
        chrome.runtime.onMessage.addListener(async function (request, sender, sendResponse) {
            ...
            if (request.instruct == "siyou_360doc") {
                console.log("1")
                click_html()
                await delay(500);
                write_code(title = request.title, content = request.content);
...
async function yuanchuang() {
    let ele = $('input#yctg')?.[0]
    if (ele) {
        ele.click()
        console.log(`点击了${ele}`);
    }
    await delay(500)
    let ele2 = $('a.rbtn_confirm_s.font12')?.[0]
    if (ele2) {
        ele2.click()
        console.log(`点击了${ele2}`);
    }
}


// 私有
async function siyou() {
    // $('input#yctg')[0].click()
    // await delay(500)
    let ele = $(`div.limit a:contains(私有)`)?.[0]
    if (ele) {
        ele.click()
        console.log(`点击了${ele}`);
